		...
		stdin:read_start(function(_, data)
			if data then
				...
				if len >= 8 and data:sub(len, len) == "t" and data:sub(1, 4) == "\x1b[4;" then
					...
					local idx = data:find(";")
					if idx then
						image.win_h, image.win_w, image.cols, image.rows =
							tonumber(data:sub(1, idx - 1)), tonumber(data:sub(idx + 1)), terminal.size()
						image.cell_w, image.cell_h =
							math.floor(image.win_w / image.cols), math.floor(image.win_h / image.rows)
